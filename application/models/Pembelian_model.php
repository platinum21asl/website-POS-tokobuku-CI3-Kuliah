...
class Pembelian_model extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT * FROM book INNER JOIN 
        book_author ON book.book_id=book_author.book_id INNER JOIN author ON 
    ...
    {
        $query = $this->db->query("SELECT pembelian.pembelian_id, pembelian.kode_beli, supplier.supplier_id, supplier.name as supplier_name, employee.employee_id, employee.name as employee_name, pembelian.buy_date FROM pembelian LEFT JOIN supplier ON pembelian.supplier_id=supplier.supplier_id INNER JOIN employee ON pembelian.employee_id=employee.employee_id");
    ...
    // menarik data dr tabel pembelian, supplierr, dan employee sesuai rentang tanggal dari inputan user
    {
        $query = $this->db->query("SELECT pembelian.pembelian_id, pembelian.kode_beli, supplier.supplier_id,supplier.name as supplier_name, employee.employee_id, employee.name as employee_name, pembelian.buy_date FROM pembelian LEFT JOIN supplier ON pembelian.supplier_id=supplier.supplier_id INNER JOIN employee ON pembelian.employee_id=employee.employee_id WHERE pembelian.buy_date >= '$start_date' AND pembelian.buy_date <= '$end_date'");
        return $query;
    }
    public function monthChart($start_date, $end_date) // menghitung jml buku terjual per bulan (current year)
    {
        $query = $this->db->query("SELECT pembelian.pembelian_id, d_pembelian.pembelian_id, SUM(d_pembelian.amount) as count, MONTHNAME(buy_date) as month_name, YEAR( CURDATE()) as year_num FROM pembelian INNER JOIN d_pembelian ON d_pembelian.pembelian_id=pembelian.pembelian_id WHERE buy_date >= '$start_date' AND buy_date <= '$end_date' GROUP BY YEAR(buy_date),MONTH(buy_date)");
        return $query;
    }
}
