...
class Penjualan_model extends CI_Model
{
    ...
    // menarik data dr tabel penjualan, customer, dan employee sesuai rentang tanggal dari inputan user
    {

        $query = $this->db->query("SELECT penjualan.penjualan_id, penjualan.kode_jual, customer.customer_id, customer.name as customer_name, employee.employee_id, employee.name as employee_name, penjualan.sale_date FROM penjualan LEFT JOIN customer ON penjualan.customer_id = customer.customer_id INNER JOIN employee ON penjualan.employee_id = employee.employee_id WHERE penjualan.sale_date >= '$start_date' AND penjualan.sale_date <= '$end_date'");

        return $query;
    }
    public function monthChart($start_date, $end_date) // menghitung jml buku terjual per bulan (current year)
    {
        $query = $this->db->query("SELECT penjualan.penjualan_id, d_penjualan.penjualan_id, SUM(d_penjualan.amount) as count, MONTHNAME(sale_date) as month_name, YEAR( CURDATE()) as year_num FROM penjualan INNER JOIN d_penjualan ON d_penjualan.penjualan_id=penjualan.penjualan_id WHERE sale_date >= '$start_date' AND sale_date <= '$end_date' GROUP BY YEAR(sale_date),MONTH(sale_date)");
        return $query;
    }
}
