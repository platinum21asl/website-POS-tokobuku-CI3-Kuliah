...
class Penjualan extends CI_Controller
{
	...
	public function show_cart()
	{
		...
		foreach ($this->cart->contents() as $items) {
			if (number_format($items['status']) == 1) {
				...
				$output .= '
 				...
 				<td> <button type="button" id="' . $items['rowid'] . '" 
				class="remove_cart btn btn-danger btn-sm">Cancel</button></td>
 				</tr>';
			} else {
				$x++;
			}
		...
		if ($no == 0 && $x == 0) {
			...
			echo '<script language=JavaScript>alert("Library Cart masih digunakan di Pembelian. Tekan `Cancel` pada bagian bawah halaman Pembelian. "); onclick=location.href="admin/pembelian"</script>';
		}
		return $output;
	}
	...
	public function clear_cart()
	{
		...
		echo '<script language=JavaScript>alert("Cancel cart"); onclick=location.href = document.referrer</script>';
	}
	...
	public function list_penjualan()
	{
		if ($this->session->userdata('employee_id') != Null) {
			...
			echo '<script language=JavaScript>onclick=location.href="../Penjualan"</script>';
		}
	}
	...
	public function pdf()
	{
		...
		$this->data = [
			'title_pdf' => 'Laporan Penjualan Toko Buku',
